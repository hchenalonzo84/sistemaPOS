...
def compras(request,compra_id=None):
    ...
    if request.method=='POST':
        ...
        if not compra_id:
            ...
            if enc:
                enc.save()
                compra_id=enc.id if enc.id else compra_id  # Asegura que se está asignando correctamente
        else:
            enc=ComprasEnc.objects.filter(pk=compra_id).first()
            if enc:
        ...
        if compra_id:
            ...
            # Si no se ha guardado correctamente, maneja el error o muestra un mensaje al usuario
            return redirect("cmp:compras_list")  # O redirige a otra vista
